	...
	string&				error)
{
	...
	for (p = 0; p < nump; ++p)
	{
		...
		for (i = 0; i < nums; ++i)
		{
			fd.myR(i)   = r;
			fd.myMu(i)  = mu*s(i);
			fd.myVar(i) = kInlines::sqr(sigma*s(i));
		}
		...
		for (h = numt - 1; h >= 0; --h)
		{
			fd.rollBwd(dt, update || h == (numt - 1), theta, wind, fd.myRes);
			if (ea > 0)
			{
				for (i = 0; i < nums; ++i) fd.myRes(0)(i) = max(res(i), fd.myRes()(0)(i));
			}
		}
	}
