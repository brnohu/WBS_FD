...
#include <Utility/kFd2d.h>
#include <Utility/kInlines.h>
	...
	LPXLOPER12 tech_in)
{
	...
	for(int n=0;n<numt;++n)
	{
		if(fb<=0)
		{
			fd.rollBwd(dt,true,theta,wind,fd.myRes);
		}
		else
		{
			fd.rollFwd(dt,true,theta,wind,fd.myRes);
		}
	}

	LPXLOPER12 out = TempXLOPER12();
	kXlUtils::setVector(fd.myRes[0], out);
	return out;
}
	...
	LPXLOPER12 tech_)
{
	...
	//	get t 
	double t;
	if (!kXlUtils::getDbl(t_, 0, 0, t, &err)) return kXlUtils::setError(err);

	//	get tech
	kVector<double> tech;
	if(!kXlUtils::getVector(tech_, tech)) return kXlUtils::setError("input tech is not a vector");

	//	standard data
	int numt  = tech.size()>0 ? (int)std::lround(tech(0)) :  1;
	int fb    = tech.size()>1 ? (int)std::lround(tech(1)) : -1;
	int windX = tech.size()>2 ? (int)std::lround(tech(2)) :  0;
	int	windY = tech.size()>3 ? (int)std::lround(tech(3)) :  0;

	//	fd grid
	kFd2d<double> fd;
	if(!kXlUtils::getVector(x_,fd.myX)) return kXlUtils::setError("input x is not a vector");
	if(!kXlUtils::getVector(y_,fd.myY)) return kXlUtils::setError("input y is not a vector");

	//	get parameters
	if(!kXlUtils::getMatrix(r_,		 fd.myR))		return kXlUtils::setError("input r is not a matrix");
	if(!kXlUtils::getMatrix(muX_,    fd.myMuX))		return kXlUtils::setError("input mu x is not a matrix");
	if(!kXlUtils::getMatrix(sigmaX_, fd.myVarX))	return kXlUtils::setError("input sigma x is not a matrix");
	if(!kXlUtils::getMatrix(muY_,    fd.myMuY))		return kXlUtils::setError("input mu y is not a matrix");
	if(!kXlUtils::getMatrix(sigmaY_, fd.myVarY))	return kXlUtils::setError("input sigma y is not a matrix");
	if(!kXlUtils::getMatrix(corrXY_, fd.myCovXY))	return kXlUtils::setError("input corr is not a matrix");

	//	get res
	fd.myRes.resize(1);
	if(!kXlUtils::getMatrix(value_, fd.myRes(0))) return kXlUtils::setError("input value is not a matrix");

	//	init and resize
	fd.init(1, fd.myX, fd.myY);
	fd.initFromSigmaRho(fd.myVarX, fd.myVarY, fd.myCovXY);
	...
	for(int i=0;i<numt;++i)
	{
		if(fb<=0)
		{
			fd.rollBwd(dt, windX, windY, fd.myRes);
		}
		else
		{
			fd.rollFwd(dt, windX, windY, fd.myRes);
		}
	}

	//	set output
	LPXLOPER12 out = TempXLOPER12();
	kXlUtils::setMatrix(fd.myRes[0], out);
	return out;
}
...
extern "C" __declspec(dllexport) int xlAutoOpen(void)
{
	...
	Excel12f(xlfRegister, 0, 11, (LPXLOPER12)&xDLL,
		...
		(LPXLOPER12)TempStr12(L"xFd2d"),
		(LPXLOPER12)TempStr12(L"t, x, y, r, muX, sigmaX, muY, sigmaY, corrXY, value0, tech"),
		(LPXLOPER12)TempStr12(L"1"),
		(LPXLOPER12)TempStr12(L"myOwnCppFunctions"),
		(LPXLOPER12)TempStr12(L""),
		(LPXLOPER12)TempStr12(L""),
